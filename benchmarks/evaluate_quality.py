...
try:
    ...
    print("Error: 'datasets' library not found. Run: uv run --with datasets python benchmarks/evaluate_quality.py")
    sys.exit(1)
...
def evaluate(num_samples: int, use_ml: bool):
    logger.info(f"Loading ai4privacy/pii-masking-200k (English subset)...")
    ...
    if use_ml:
        ...
        if not onnx_model_path.exists() or not tokenizer_path.exists():
            logger.error(f"ML artifacts not found in {CACHE_DIR}. Please run 'uv run pytest tests/unit/backends/test_onnx.py' first to download them.")
            sys.exit(1)
    ...
    for row in ds:
        ...
        for d_start, d_end in detected_spans:
            matched = False
            for i, (g_start, g_end, g_label) in enumerate(ground_truth_spans):
                # Check overlap
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate pseudonymize precision and recall against real-world datasets.")
    parser.add_argument("--samples", type=int, default=500, help="Number of samples to evaluate.")
    parser.add_argument("--ml", action="store_true", help="Include the ONNX ML backend in evaluation.")
