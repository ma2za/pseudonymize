...
def main() -> None:
    ...
    with open("benchmark_results_raw.txt", "w", encoding="utf-8") as out_file:
        ...
        for model_info in MODELS:
            ...
            try:
                cache_dir = snapshot_download(model_info['repo'], allow_patterns=["config.json", "tokenizer.json", model_info['onnx_file']])
                ...
                eq.LocalONNXPIIBackend = lambda **kwargs: LocalONNXPIIBackend(
                    model_path=onnx_path, tokenizer_path=tokenizer_path, config_path=config_path if config_path.exists() else None
                )
