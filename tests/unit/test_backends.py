...
def test_composite_backend_ensemble_modes() -> None:
    from pseudonymize import EnsembleMode

    # Set up two stub backends:
    # - backend_a finds a PERSON "John Smith" at [0, 10] and LOCATION "Seattle" at [15, 21]
    # - backend_b finds a PERSON "John Smith" at [0, 10] (overlap/intersection)
    block = ContentBlock("body", "John Smith in Seattle", TextOffsetLocation(0, 21))
    policy = Policy()

    det_person_a = Detection(EntityType.PERSON, 0, 10, 0.99, "stub_a")
    det_loc_a = Detection(EntityType.LOCATION, 15, 21, 0.90, "stub_a")
    det_person_b = Detection(EntityType.PERSON, 0, 10, 0.95, "stub_b")

    backend_a = StubBackend("a", [det_person_a, det_loc_a])
    backend_b = StubBackend("b", [det_person_b])

    # 1. UNION mode (default): should keep both PERSON and LOCATION
    union_backend = CompositeBackend(
        backends=[backend_a, backend_b],
        ensemble_mode=EnsembleMode.UNION,
    )
    union_res = union_backend.detect(block, policy)
    assert len(union_res) == 2
    types = {d.entity_type for d in union_res}
    assert EntityType.PERSON in types
    assert EntityType.LOCATION in types

    # 2. INTERSECTION mode: should ONLY keep PERSON since LOCATION is only found by backend_a
    intersection_backend = CompositeBackend(
        backends=[backend_a, backend_b],
        ensemble_mode=EnsembleMode.INTERSECTION,
    )
    intersection_res = intersection_backend.detect(block, policy)
    assert len(intersection_res) == 1
    assert intersection_res[0].entity_type == EntityType.PERSON

    # 3. Integrating with Pseudonymizer (Non-flattened CompositeBackend check)
    engine = Pseudonymizer(
        backends=[intersection_backend],
    )
    res = engine.detect("John Smith in Seattle")
    assert len(res) == 1
    assert res[0].entity_type == EntityType.PERSON
