...
def _write_project(root: Path, version: str = "0.1.0a1") -> None:
    (root / "pyproject.toml").write_text(
        f'[project]\nname = "pseudonymize"\nversion = "{version}"\n', encoding="utf-8"
    )


def _write_wheel(directory: Path, version: str = "0.1.0a1", dependency: bool = False) -> None:
...
def _write_sdist(directory: Path, version: str = "0.1.0a1") -> None:
...
def test_release_artifacts_and_tag(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    verify_release(tmp_path, distribution_directory, "v0.1.0a1")
    assert "dependency-free" in capsys.readouterr().out
    assert project_version(tmp_path / "pyproject.toml") == "0.1.0a1"


def test_release_rejects_mismatched_tag() -> None:
    with pytest.raises(ValueError, match="does not match"):
        verify_tag("0.1.0a1", "v0.1.0a2")
