      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
        <form className="space-y-6 bg-[var(--pz-surface)] border border-[var(--pz-border)] p-6 sm:p-8 rounded-xl shadow-lg" onSubmit={handleSignup}>
          <div>
            ...
            <div className="mt-2">
              <input id="name-input" type="text" required className="block w-full rounded-md border border-[var(--pz-border-strong)] py-2.5 text-[var(--pz-text)] bg-[var(--pz-surface-inset)] shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--pz-cipher)] focus:border-transparent sm:text-sm sm:leading-6 px-4 transition-all" value={name} onChange={e => setName(e.target.value)} disabled={loading} />
            </div>
          </div>
          ...
          <div className="pt-2">
            <button type="submit" disabled={loading} className="flex w-full justify-center rounded-md bg-[var(--pz-cipher)] px-4 py-2.5 text-sm font-semibold leading-6 text-[var(--pz-ink)] shadow-sm hover:bg-[var(--pz-cipher-hover)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-50 transition-colors">{loading ? '...' : t('signUpButton')}</button>
          </div>
        </form>
        ...
        <div className="mt-8">
          ...
          <div className="mt-6">
            <button onClick={handleGoogleSignup} disabled={loading} className="flex w-full justify-center items-center gap-3 rounded-md bg-[var(--pz-surface)] px-4 py-2.5 text-sm font-semibold text-[var(--pz-text)] shadow-sm border border-[var(--pz-border-strong)] hover:bg-[var(--pz-surface-inset)] transition-colors disabled:opacity-50">
